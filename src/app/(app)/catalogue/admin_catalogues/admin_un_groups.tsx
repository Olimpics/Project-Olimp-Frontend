...
export const AdminGroupsCatalogue = () => {
  ...
  const fetchFilteredData = useCallback(async (page: number = currentPage) => {
    ...
    const res = await fetch(
      `http://212.3.125.183:7011/api/Group?${query.toString()}`
    )
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      const deptData = await (await fetch('http://212.3.125.183:7011/api/Department?page=1&pageSize=50')).json()
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(`http://212.3.125.183:7011/api/Group/${selectedGroup.id}`, {
        method: 'DELETE'
      })
