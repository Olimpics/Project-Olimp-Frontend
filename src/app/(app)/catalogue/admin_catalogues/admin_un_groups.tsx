...
export const AdminGroupsCatalogue = () => {
  ...
  const fetchFilteredData = useCallback(async (page: number = currentPage) => {
    ...
    const res = await fetch(
      `http://localhost:5154/api/Group?${query.toString()}`
    )
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      const deptData = await (await fetch('http://localhost:5154/api/Department?page=1&pageSize=50')).json()
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5154/api/Group/${selectedGroup.id}`, {
        method: 'DELETE'
      })
