...
export const AdminDepartmentCatalogue = () => {
  ...
  const fetchDepartments = useCallback(async (page: number = currentPage) => {
    ...
    try {
      ...
      const res = await fetch(`http://212.3.125.183:7011/api/Department?${query.toString()}`)
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(`http://212.3.125.183:7011/api/Department/${selectedDepartment.idDepartment}`, {
        method: 'DELETE'
      })
  ...
  const addDepartment = async () => {
    try {
      const response = await fetch('http://212.3.125.183:7011/api/Department', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
