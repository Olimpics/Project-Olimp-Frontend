...
export const AdminDepartmentCatalogue = () => {
  ...
  const fetchDepartments = useCallback(async (page: number = currentPage) => {
    ...
    try {
      ...
      const res = await fetch(`https://localhost:5154/api/Department?${query.toString()}`)
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(`https://localhost:5154/api/Department/${selectedDepartment.idDepartment}`, {
        method: 'DELETE'
      })
  ...
  const addDepartment = async () => {
    try {
      const response = await fetch('https://localhost:5154/api/Department', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
