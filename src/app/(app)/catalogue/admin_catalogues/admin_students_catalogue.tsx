...
export const AdminStudentCatalogue = () => {
  ...
  const fetchFilteredData = useCallback(async (page: number = currentPage) => {
    ...
    if (groupGive) {
      try {
        const groupRes = await fetch('http://212.3.125.183:7011/api/Filter/groups');
        if (groupRes.ok) {
    ...
    try {
      const res = await fetch(`http://212.3.125.183:7011/api/Student?${query.toString()}`);
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      const facData = await (await fetch('http://212.3.125.183:7011/api/Faculty')).json()
      const eduData = await (await fetch('http://212.3.125.183:7011/api/EducationalDegree')).json()
      const specData = await (await fetch('http://212.3.125.183:7011/api/Filter/specialities')).json()
      const groupData = await (await fetch('http://212.3.125.183:7011/api/Filter/groups')).json()
      const studyFormData = await (await fetch('http://212.3.125.183:7011/api/StudyForm')).json()
  ...
  const confirmDelete = async () => {
    ...
    const response = await fetch(`http://212.3.125.183:7011/api/Student/${selectedStudent.idStudents}`, {
      method: 'DELETE'
    })
    ...
    <div className="p-4 sm:p-6 bg-gray-100 min-h-screen flex flex-col sm:flex-row gap-4">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
        ...
        {modalType === 'delete' && selectedStudent && (
          <div>
            <h2 className="text-xl font-bold mb-4">Видалення студента</h2>
            <p>Ви впевнені, що хочете видалити студента {selectedStudent.nameStudent}?</p>
