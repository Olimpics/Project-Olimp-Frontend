...
export const AdminStudentCatalogue = () => {
  ...
  const fetchFilteredData = useCallback(async (page: number = currentPage) => {
    ...
    if (groupGive) {
      try {
        const groupRes = await fetch('https://localhost:5154/api/Filter/groups');
        if (groupRes.ok) {
    ...
    try {
      const res = await fetch(`https://localhost:5154/api/Student?${query.toString()}`);
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      const specData = await (await fetch('https://localhost:5154/api/Filter/specialities')).json()
      const groupData = await (await fetch('https://localhost:5154/api/Filter/groups')).json()
      const studyFormData = await (await fetch('https://localhost:5154/api/StudyForm')).json()
  ...
  const confirmDelete = async () => {
    ...
    const response = await fetch(`https://localhost:5154/api/Student/${selectedStudent.idStudents}`, {
      method: 'DELETE'
    })
