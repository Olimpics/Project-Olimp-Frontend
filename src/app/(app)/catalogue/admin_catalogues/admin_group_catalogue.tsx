...
export const AdminEducationalProgramCatalogue = () => {
  ...
  const fetchPrograms = useCallback(async (page: number = currentPage) => {
    ...
    try {
      ...
      const res = await fetch(
        `http://212.3.125.183:7011/api/EducationalProgram?${query.toString()}`,
        {
          headers: {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        const degreesData = await (await fetch('http://212.3.125.183:7011/api/EducationalDegree')).json()
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(
        `http://212.3.125.183:7011/api/EducationalProgram/${selectedProgram.idEducationalProgram}`,
        {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
  ...
  const addProgram = async () => {
    ...
    try {
      const response = await fetch('http://212.3.125.183:7011/api/EducationalProgram', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
