...
export const AdminEducationalProgramCatalogue = () => {
  ...
  const fetchPrograms = useCallback(async (page: number = currentPage) => {
    ...
    try {
      ...
      const res = await fetch(
        `https://localhost:5154/api/EducationalProgram?${query.toString()}`,
        {
          headers: {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        const degreesData = await (await fetch('https://localhost:5154/api/EducationalDegree')).json()
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(
        `https://localhost:5154/api/EducationalProgram/${selectedProgram.idEducationalProgram}`,
        {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
  ...
  const addProgram = async () => {
    ...
    try {
      const response = await fetch('https://localhost:5154/api/EducationalProgram', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
