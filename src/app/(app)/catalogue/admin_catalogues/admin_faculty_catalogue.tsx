...
export const AdminFacultyCatalogue = () => {
  ...
  const fetchFaculties = useCallback(async (page: number = currentPage) => {
    ...
    try {
      ...
      const res = await fetch(`https://localhost:5154/api/Faculty?${query.toString()}`)
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(`https://localhost:5154/api/Faculty/${selectedFaculty.idFaculty}`, {
        method: 'DELETE'
      })
  ...
  const addFaculty = async () => {
    try {
      const response = await fetch('https://localhost:5154/api/Faculty', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
