...
export const AdminBindLoansPage = () => {
  ...
  const fetchFilteredData = useCallback(async (page: number = currentPage) => {
    ...
    try {
      const res = await fetchWithAuth(
        `http://212.3.125.183:7011/api/BindLoansMain?${query.toString()}`
      )
    ...
    const fetchInitialData = async () => {
      ...
      try {
        const [disciplinesRes, specialitiesRes] = await Promise.all([
          fetchWithAuth('http://212.3.125.183:7011/api/Filter/add-disciplines'), 
          fetchWithAuth('http://212.3.125.183:7011/api/Filter/specialities')
        ])
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetchWithAuth(
        `http://212.3.125.183:7011/api/BindLoansMain/${selectedBindLoan.idBindLoan}`,
        { method: 'DELETE' }
      )
  ...
  const saveChanges = async () => {
    ...
    try {
      let response
      if (modalType === 'add') {
        response = await fetchWithAuth('http://212.3.125.183:7011/api/BindLoansMain', {
          method: 'POST',
          headers: {
