...
export const StudentDisciplinesCatalogue = React.memo(() => {
  ...
  const fetchFilteredData = useCallback(
    async (page: number = currentPage) => {
      ...
      const res = await fetch(
        `http://localhost:5154/api/DisciplineTabStudent/GetAllDisciplinesWithAvailability?${query.toString()}`
      )
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      const facData = await (
        await fetch('http://localhost:5154/api/Faculty')
      ).json()
      const eduData = await (
        await fetch('http://localhost:5154/api/EducationalDegree')
      ).json()
