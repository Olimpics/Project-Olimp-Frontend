...
const CourseCataloguePage = () => {
    ...
    async (page: number = currentPage) => {
      ...
      try {
        ...
        const res = await fetch(
          `https://localhost:5154/api/DisciplineTabAdmin/GetStudentsWithDisciplineChoices?${params.toString()}`
        )
        if (!res.ok) {
    ...
    const fetchFilters = async () => {
      try {
        const [facRes, degRes, groupRes] = await Promise.all([
          ...
          fetch('https://localhost:5154/api/Filter/groups'),
        ])
  ...
  const performSave = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch('https://localhost:5154/api/DisciplineTabAdmin/UpdateChoice', {
        method: 'PUT',
