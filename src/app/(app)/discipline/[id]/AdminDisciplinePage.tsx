...
export default function AdminDisciplinePage({ id }: { id: string }) {
  ...
  const handleUpdate = async () => {
    try {
      ...
      const response = await fetch('http://212.3.125.183:5154/api/DisciplineTabStudent/UpdateDisciplineWithDetails', {
        method: 'POST',
        headers: { 
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pb-12 font-sans">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <DisciplineBlock title="Основна інформація">
                <div className="grid grid-cols-2 gap-x-8 gap-y-6">
                  ...
                  <InfoBlock label="Мова" value={discipline.language} />
                ...
                <div className="mt-8 pt-8 border-t border-gray-50">
                  <div className="flex flex-col md:flex-row md:items-center gap-4">
                    <div className="flex items-center gap-4 min-w-[200px]">
                      ...
                      <button onClick={() => setIsRecommendedModalOpen(true)} className="text-[11px] font-bold text-[#1e50f0] bg-blue-50 px-2.5 py-1 rounded-lg hover:bg-blue-100 transition-all whitespace-nowrap">Рекомендовано</button>
                    </div>
              ...
              <DisciplineBlock title="Опис дисципліни">
                <div className="space-y-6">
                  <div className="space-y-2"><h4 className="text-sm text-gray-400">Що вивчатиметься</h4><p className="text-[15px] font-bold text-gray-900 leading-relaxed">{discipline.determination}</p></div>
                  <div className="space-y-2"><h4 className="text-sm text-gray-400">Чому важливо</h4><p className="text-[15px] font-bold text-gray-900 leading-relaxed">{discipline.whyInterestingDetermination}</p></div>
                </div>
              </DisciplineBlock>
            </div>

            <DisciplineBlock title="Результати та компетенції">
              <div className="space-y-8">
                <div className="space-y-2"><h4 className="text-sm text-gray-400 font-medium">Результати навчання</h4><p className="text-[15px] font-medium text-gray-600 leading-relaxed">{discipline.resultEducation}</p></div>
                <div className="space-y-2"><h4 className="text-sm text-gray-400 font-medium">Компетенції</h4><p className="text-[15px] font-medium text-gray-600 leading-relaxed">{discipline.usingIrl}</p></div>
          ...
          <div className="bg-white rounded-[24px] border border-gray-100 shadow-sm overflow-hidden">
            <div className="p-6 flex justify-between items-center bg-gray-50/50 border-b border-gray-100">
              <div className="flex items-center gap-4">
                <h3 className="text-lg font-bold text-gray-900">Студенти на курсі</h3>
              ...
              <button onClick={() => { setIsAddModalOpen(true); fetchAvailableStudents(); }}
                className="bg-green-600 text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-md hover:bg-green-700 transition-all active:scale-95 flex items-center gap-2">
      ...
      <Modal isOpen={isRecommendedModalOpen} onClose={() => setIsRecommendedModalOpen(false)} classSize="max-w-4xl">
        <div className="p-8 space-y-6">
          <div className="flex justify-between items-center border-b pb-4">
            <h2 className="text-xl font-bold text-gray-900">Рекомендовано для</h2>
          ...
          <div className="grid grid-cols-2 gap-6">
            <SelectionColumn title="Спеціальності" options={specialties} selectedIds={selectedSpecialties}
          ...
          <div className="flex justify-end gap-3 pt-6 border-t">
            ...
            }} className="px-8 py-2.5 bg-[#1e50f0] text-white rounded-xl font-bold shadow-lg hover:bg-blue-700 transition-all shadow-blue-200">Підтвердити вибір</button>
          </div>
        </div>
      </Modal>
      ...
      <Modal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} classSize="max-w-[700px]">
        <div className="relative">
          {/* Header */}
          ...
          <div className="p-8 space-y-8 max-h-[80vh] overflow-y-auto bg-white">
            <div className="space-y-6">
              ...
              <div className="pt-4">
                <h3 className="text-[17px] font-bold text-gray-900 mb-6">Основна інформація</h3>
                <div className="grid grid-cols-2 gap-x-6 gap-y-6">
                  ...
                  <ModalField label="Кафедра" value={editForm?.details?.departmentId} type="select" options={[{value: discipline.departmentId, label: discipline.departmentName}]} onChange={v => updateForm('details.departmentId', Number(v))} />
                  ...
                  <ModalField label="Семестр" value={editForm?.isEven} type="select" 
                    options={[{value: 1, label: 'Непарний'}, {value: 2, label: 'Парний'}, {value: 0, label: 'Обидва'}]} 
                    onChange={v => updateForm('isEven', Number(v))} />
                </div>
              </div>

              <div className="pt-4">
                <h3 className="text-[17px] font-bold text-gray-900 mb-6">Опис дисципліни</h3>
                <div className="space-y-6">
